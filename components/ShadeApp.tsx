...
function mergeQuests(stored: Quest[]) {
  return defaultQuests.map((quest) => ({ ...quest, ...stored.find((item) => item.id === quest.id) }));
}

function getDashboardComments(scores: CategoryScores) {
  const comments: string[] = [];

  if (scores.presence >= 70) comments.push("今日は社会空間に入れる状態です。姿勢と視線だけ維持してください。");
  else comments.push("Presenceはまだ上げ幅があります。大きく攻めず、入る店を一つだけ決めて動いてください。");

  if (scores.face < 70) comments.push("Faceの完成度が少し不安定です。睡眠・眉・肌の順で整えると修正しやすいです。");
  if (scores.food >= 75) comments.push("食事管理はかなり安定しています。見た目に出る土台は崩れていません。");
  if (scores.mind < 55) comments.push("Mindが低めです。今日は無理に攻めず、最低限の整えと記録に寄せてください。");
  if (scores.total >= 80) comments.push("全体のノイズが少ない日です。写真を残して、何が効いたか観察してください。");
...
function DifficultyBadge({ quest }: { quest: Quest }) {
  ...
  const tone = difficulty >= 4 ? "border-blue-300/40 bg-blue-300/10 text-blue-100" : difficulty >= 3 ? "border-slate-300/30 bg-slate-200/10 text-slate-100" : "border-white/10 bg-white/5 text-slate-400";
  return (
    <span className={`rounded-full border px-3 py-1 text-xs font-medium ${tone}`}>
      {quest.stars}{difficulty >= 3 ? " / 高難度" : ""}
    </span>
  );
}
...
function Delta({ value }: { value: number }) {
  const sign = value > 0 ? "+" : "";
  const color = value > 0 ? "text-blue-200" : value < 0 ? "text-slate-500" : "text-slate-400";
  return <span className={`text-sm font-semibold ${color}`}>{sign}{value}</span>;
}

function ComparisonPanel({ before, after }: { before?: DailyLog; after?: DailyLog }) {
  if (!before || !after) {
    return <p className="rounded-3xl border border-dashed border-white/10 p-6 text-sm text-slate-500">比較するログを2件選択してください。</p>;
  }
  ...
  const CardSide = ({ label, log, scores }: { label: string; log: DailyLog; scores: CategoryScores }) => (
    <div className="space-y-4 rounded-3xl border border-white/10 bg-black/20 p-4">
      <div className="flex items-center justify-between gap-3">
        ...
        <div className="rounded-2xl border border-white/10 px-4 py-2 text-2xl font-semibold text-white">{scores.total}</div>
      </div>
      <LogPhotoView photo={log.photo} className="h-64 w-full" label="No photo" />
  ...
  return (
    <div className="space-y-4">
      <div className="grid gap-4 lg:grid-cols-[1fr_auto_1fr]">
        ...
        <div className="grid place-items-center text-xs uppercase tracking-[0.26em] text-slate-500">vs</div>
      ...
      <div className="grid gap-2 rounded-3xl border border-white/10 bg-white/[0.03] p-4 sm:grid-cols-3 lg:grid-cols-6">
        {(["total", ...scoreKeys] as const).map((key) => (
          <div key={key} className="flex items-center justify-between rounded-2xl bg-black/25 px-3 py-2">
...
export default function ShadeApp() {
  ...
  useEffect(() => {
    ...
    if (!compareBeforeId) setCompareBeforeId(orderedLogs[1]?.id ?? orderedLogs[0].id);
  ...
  const handlePhotoChange = (event: ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = () => {
      ...
      const photo: LogPhoto = { dataUrl: reader.result, name: file.name, type: file.type, size: file.size };
  ...
  const completeQuest = (questId: string) => {
    setQuests((current) => current.map((item) => item.id === questId ? { ...item, completed: true, completedAt: new Date().toISOString(), completionMemo: questMemo.trim() } : item));
    setMarkingQuestId(null);
    setQuestMemo("");
  };

  const resetQuest = (questId: string) => {
    setQuests((current) => current.map((item) => item.id === questId ? { ...item, completed: false, completedAt: undefined, completionMemo: undefined } : item));
  };
      ...
      <div className="relative mx-auto flex w-full max-w-7xl flex-col gap-8 px-4 py-6 sm:px-6 lg:px-8">
        <header className="flex flex-col gap-6 rounded-[2rem] border border-white/10 bg-black/30 p-5 backdrop-blur sm:p-6 md:flex-row md:items-end md:justify-between">
          <div>
            ...
            <p className="mt-4 max-w-xl text-sm leading-7 text-slate-400">顔・髪・服・行動のログを接続し、外見と社会空間への入り方を観察する自己管理OS。</p>
          </div>
          ...
          <section className="grid gap-5 lg:grid-cols-[1.1fr_0.9fr]">
            <Card className="flex flex-col gap-8 md:flex-row md:items-center md:justify-around">
              ...
              <div className="grid flex-1 gap-3 sm:grid-cols-2">
                {latestScores && scoreKeys.map((key) => (
                  <div key={key} className="rounded-2xl border border-white/10 bg-black/25 p-4">
                    <div className="flex items-center justify-between"><span className="text-sm text-slate-400">{scoreLabels[key]}</span><span className="text-xl font-semibold text-white">{latestScores[key]}</span></div>
                    <div className="mt-3 h-1.5 rounded-full bg-white/10"><div className="h-full rounded-full bg-shade-blue" style={{ width: `${latestScores[key]}%` }} /></div>
                  </div>
                ))}
              </div>
            </Card>
            <Card>
              <p className="text-xs uppercase tracking-[0.28em] text-shade-blue/80">Today Comment</p>
              <h2 className="mt-2 text-2xl font-semibold text-white">冷静に観察する</h2>
              <div className="mt-4 space-y-3">
                {latestScores ? getDashboardComments(latestScores).map((comment) => (
                  <p key={comment} className="rounded-2xl border border-white/10 bg-black/20 p-3 text-sm leading-6 text-slate-300">{comment}</p>
                )) : <p className="text-sm text-slate-500">最初のログを保存すると、固定ロジックの状態コメントが表示されます。</p>}
              </div>
            </Card>
            ...
            <div className="space-y-4 lg:col-span-2">
              ...
              <div className="grid gap-4 md:grid-cols-2">{orderedLogs.slice(0, 4).map((log) => <LogCard key={log.id} log={log} expanded={expandedLogId === log.id} onToggle={() => setExpandedLogId(expandedLogId === log.id ? null : log.id)} />)}</div>
            </div>
          </section>
        )}
        ...
        {activeTab === "new" && (
          <section className="space-y-5">
            ...
            <Card className="grid gap-5 lg:grid-cols-[320px_1fr]">
              <LogPhotoView photo={form.photo} className="h-80 w-full" label="Photo preview" />
              <div className="space-y-4">
                <div>
                  <p className="text-xs uppercase tracking-[0.28em] text-shade-blue/80">Photo</p>
                  <h2 className="mt-1 text-2xl font-semibold text-white">顔・髪型・コーデ・雰囲気</h2>
                  <p className="mt-2 text-sm leading-6 text-slate-400">画像はData URLに変換してlocalStorageへ保存します。未登録でもログは保存できます。</p>
                </div>
                <TextInput type="file" accept="image/*" onChange={handlePhotoChange} />
                {photoError ? <p className="text-sm text-blue-200">{photoError}</p> : null}
                {form.photo ? <Button onClick={() => setForm({ ...form, photo: undefined })}>写真を外す</Button> : null}
              </div>
            </Card>
            ...
            <Card className="space-y-4">
              ...
              <Button onClick={saveLog} className="w-full justify-center bg-shade-blue/25 py-2.5">保存する</Button>
            </Card>
          </section>
        )}
              ...
              <Card key={quest.id} className="space-y-4">
                <div className="flex items-start justify-between gap-4">
                  <div>
                    ...
                    <div className="mt-2 flex flex-wrap items-center gap-2"><DifficultyBadge quest={quest} />{quest.completed ? <Pill active>Done</Pill> : null}</div>
                  </div>
                  {quest.completed ? <Button onClick={() => resetQuest(quest.id)}>Reset</Button> : <Button onClick={() => startQuestMark(quest)}>Mark</Button>}
                </div>
                {quest.completed ? (
                  <div className="rounded-2xl border border-white/10 bg-black/20 p-3 text-sm leading-6 text-slate-300">
                    <p className="text-slate-500">達成日: {quest.completedAt?.slice(0, 10) ?? "—"}</p>
                ...
                {markingQuestId === quest.id ? (
                  <div className="space-y-3 border-t border-white/10 pt-4">
                    ...
                    <TextArea placeholder="緊張したが入れた / 店員対応が普通で安心した" value={questMemo} onChange={(e) => setQuestMemo(e.target.value)} />
                    <div className="flex gap-2"><Button onClick={() => completeQuest(quest.id)} className="bg-shade-blue/20">達成保存</Button><Button onClick={() => setMarkingQuestId(null)}>Cancel</Button></div>
                  </div>
                ) : null}
              </Card>
            ))}
        ...
        {activeTab === "logs" && (
          <section className="space-y-5">
            <div className="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
              <div><p className="text-xs uppercase tracking-[0.28em] text-shade-blue/80">Archive</p><h2 className="text-3xl font-semibold text-white">Log一覧</h2></div>
              <Button onClick={() => setActiveTab("new")}>新規ログ</Button>
            </div>
            <PresenceTrend logs={orderedLogs} />
            <Card className="space-y-4">
              <div>
                <p className="text-xs uppercase tracking-[0.28em] text-shade-blue/80">Before / After</p>
                <h2 className="mt-1 text-2xl font-semibold text-white">変化を横並びで見る</h2>
              </div>
              <div className="grid gap-3 md:grid-cols-2">
                <div><FieldLabel>Before</FieldLabel><select className="w-full rounded-2xl border border-white/10 bg-black/30 px-3.5 py-2.5 text-sm text-slate-100 outline-none" value={compareBeforeId} onChange={(e) => setCompareBeforeId(e.target.value)}>{orderedLogs.map((log) => <option key={log.id} value={log.id}>{log.date}</option>)}</select></div>
                <div><FieldLabel>After</FieldLabel><select className="w-full rounded-2xl border border-white/10 bg-black/30 px-3.5 py-2.5 text-sm text-slate-100 outline-none" value={compareAfterId} onChange={(e) => setCompareAfterId(e.target.value)}>{orderedLogs.map((log) => <option key={log.id} value={log.id}>{log.date}</option>)}</select></div>
              </div>
